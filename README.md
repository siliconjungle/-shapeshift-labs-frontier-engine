...
### `createSchemaPatcher(options)`

Creates a small patcher for a fixed schema/profile. This is for hot paths where the JSON shape is known from JSON Schema, query/table shape metadata, or a trusted engine profile.

```ts
const patcher = createSchemaPatcher({
  schema: {
    type: 'array',
    path: ['todos'],
    key: 'id',
    item: { type: 'object', fields: ['id', 'done', 'title'] }
  }
});

const patch = patcher.diff(before, after);
patcher.diffInto(before, after, reusable);
```

By default the patcher falls back to generic diffing if a value no longer matches the compiled shape. Pass `fallback: false` when a shape miss should be treated as a programmer error.
...
import { createSchemaPatcher } from '@shapeshift-labs/frontier-engine/patcher';
...
- Schema-compiled patchers for fixed JSON shapes.
...
The package test suite covers root and subpath imports, schema diff/apply replay, schema patchers, profile snapshots, optional numeric quantization, history planning, encoded history replay, and the absence of state/CRDT exports. The fuzzer covers schema, schema-patcher, and adaptive profile round-trips over record-array and object-shaped JSON.
...
| Fixture | Median | p95 |
| --- | ---: | ---: |
| Core diff, 1k rows with arrayKey | 266.26 us | 271.49 us |
| Engine schema diff, 1k rows | 13.43 us | 13.95 us |
| Engine quantized schema diff, 1k rows | 150.01 us | 154.92 us |
| Engine quantized drift no-op, 1k rows | 150.22 us | 161.03 us |
| Schema patcher diff, 1k rows | 13.52 us | 14.75 us |
| Schema patcher diffInto, 1k rows | 13.44 us | 14.32 us |
| Engine apply via core patch | 0.51 us | 0.87 us |
| Engine equality no-op | 9.77 us | 11.92 us |
| Schema patcher equality no-op | 10.14 us | 11.80 us |
| Engine history encode/decode/apply | 3.86 us | 5.16 us |
